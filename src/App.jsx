  ...
  return (
    <div className={key?'darkWrapper wrapper pt-5 ':'wrapper pt-5'}>
      <div className={key? 'darkMode card mx-auto px-2':'card mx-auto px-2'}>
        <span className='text-light'>Night mode</span>
        <Switch onClick={()=>toggle()}/>
        <input value={name} onChange={(e) => setName(e.target.value)} type="text" className=' form-control w-75 mx-auto mt-4 form-control-lg' placeholder='Yangi maxsulot nomi...' />
        <input value={cost} onChange={(e) => setCost(e.target.value)} type="text" className=' form-control w-75 mx-auto mt-2 form-control-lg' placeholder='Yangi maxsulot narxi...' />
        <Btn onClick={() => addProduct()} className='btn btn-success w-75 mx-auto my-3 btn-lg' />
        {
          product.length ?
            <table className='table table-bordered border-success text-center mt-2'>
              <Head />
              <tbody style={key? {color:"#fff"} : {color:"#000"}}>
